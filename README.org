...
#+BEGIN_SRC shell :var zmk_path="../zmk"
  ZMK_CONFIG_PATH="$zmk_path/../zmk-config/"
  LABEL=zmk-dev
  ...
  devcontainer up --id-label=$LABEL --workspace-folder "$zmk_path"

  echo "- Initial setup in the container..."
  docker exec --id-label=$LABEL west init -l /app
  docker exec --id-label=$LABEL west update
  # docker exec --id-label=$LABEL west zephyr-export

  # echo "- Install LSP Server (clangd) for emacs..."
  # docker exec --id-label=$LABEL pip3 install pre-commit
#+END_SRC
